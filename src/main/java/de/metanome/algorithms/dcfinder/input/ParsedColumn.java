...
public class ParsedColumn<T extends Comparable<T>> {
    ...
    public double[] getSharedPercentage2(ParsedColumn<?> c2) {
        int totalCount = 0;
        int sharedCount = 0;
        for (var e : valueSet.entrySet()) {
            int thisCount = e.getValue();
        ...
        return new double[]{((double) sharedCount) / ((double) totalCount),Double.valueOf(totalCount),Double.valueOf(sharedCount)};
    }
