...
class ApproxCoverNode {
    ...
    static void configure(int nEle) {
    ...
    HyperEdge chooseAnCanHitUncovEdge() {
        Comparator<HyperEdge> cmp = Comparator.comparing(e -> LongBitSet.getAndCardinality(nontrivialCand, e.bitset));
    ...
    ApproxCoverNode getUnhitChild(HyperEdge unhitEdge) {
        LongBitSet childCand = nontrivialCand.getAndNot(unhitEdge.bitset);
        List<HyperEdge> childCanHitUncov = new ArrayList<>();

        long bestTarget = target;
        for (HyperEdge e : canHitUncov) {
            if (childCand.intersectWith(e.bitset)) {
    ...
    ApproxCoverNode getHitChild(int addV, LongBitSet childCand) {
        ...
        LongBitSet childNontrivialCand = childCand.clone();
